 ...
 * */
open class FramefulEasePlayerJavaBiFunction<T : Number>(
    private val lower: T,
    private val upper: T,
    private val numberOfFrames: Int,
    private val easeFactory: BiFunction<T, T, Ease<T>>
) : EasePlayer<T>, Iterator<T> {

    /**
     * The current ease function
     * */
    override var easeFn: Ease<T> = easeFactory.apply(lower, upper)
    ...
    override var transitionTo: T = upper
        set(value) {
            ...
            field = value
            easeFn = easeFactory.apply(lower, upper)
            reset()
        }
